...
void performOP(struct code cd[])
{
	...
	struct TII tii[10];
	int i = 0, flag = 0, constant = 1, locMN, fl = 0, poolno = 0, poolflag = 0, litflag = 0, count = 0, tiiCNT = 0, symCNT = 0, cflag=0, eflag = 0, equind = 0, endflag = 0, forpooltab = 1;
	char pool;
	queue <string> q1;
 		...
 		strcpy(s, tp.c_str());	
   		...
   		while(token) 
			...
			else
			{
				if(flag)
				{
					...
					else if(s == 'S')
					{
						...
						else
						{
							...
							{
								cout<<"\n**ERROR** Duplicate entry of variable\n"<<token<<endl;
								break;
							}
						}
				...
				{
					cout<<"\n**ERROR** Program must start with AD START";
					break;
				}	
			}
	...
	for(int i=0;i<tiiCNT;i++)
	{
	  ...
	  if(ERRORflag == 0)
	  {
	    cout<<"**ERROR** Variable declaration not found...:"<<tii[i].name<<"\n";
	    //break;
	  }
	  ERRORflag = 0;
	}
